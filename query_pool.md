...
采用“**payment 负责业务状态机与执行调度**”的一体化实现：
...
该方案保证“规则 + 执行”在同一服务内闭环，减少跨服务协议与一致性成本。
...
2. `payment` 按 `next_run_at` 拉取可执行任务。
3. `payment` worker 并发执行插件 `Query`。
...
执行层也在 `payment` 内部，但仍应保持“执行器不承载业务语义、由策略模块解释配置”。

---

## 8. 结果落库契约

`payment` 执行 Query 后，最小应记录：
